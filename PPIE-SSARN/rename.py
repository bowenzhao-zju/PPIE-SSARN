...
def main() -> object:
    ...
    checkpoint.update({'RCAN_MS':checkpoint.pop("PPIE_SSARN")})
    checkpoint.update({'ddf':checkpoint.pop("ddmf")})
    print(checkpoint.items())

    # state = {"epoch": 3727,"model": new_model_state_dict}
    # torch.save(state, 'new_model.pth')
